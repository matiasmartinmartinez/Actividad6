...
4.
```{r,echo=TRUE}
filter(happy,!is.na(happy)) %>% group_by(sex,happy,age) %>% summarise(cont=n()) %>%arrange(age) %>% group_by(age,sex) %>% mutate(porcent=(cont/sum(cont)*100)) %>% ggplot(aes(x=age,y=porcent,colour=happy))+geom_area()+labs(x="Edad",y="Porcentaje",title="Gráfico de dispersión: Porcentaje niveles de felicidad según edad")+facet_wrap(~sex) + theme_bw()
```
